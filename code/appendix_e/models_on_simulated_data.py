...
parser.add_argument('model', choices=['prototype', 'exemplar'], help='either "prototype" or "exemplar"')
parser.add_argument('-a', '--after_all_seen', action='store_true', help='whether to only include trials after the last exemplar was introduced')

args = parser.parse_args()

this_model = sys.argv[1]
...
with open(f"../../data/{this_model}_simulated_data.csv") as fp:
...
for response in responses:
	...
	for bin in bins:
		...
		for trial in bin:
			seen = []
			if args.after_all_seen and trial < sorted_stimuli[13][1]:
				continue
			...
			denom += 1
		results_exemp[bin_num].append(res_exemp / denom)
		results_proto[bin_num].append(res_proto /  denom)
		bin_num += 1
