...
def resid_calc_exemp(x, stim, data, seen, last_seen=None, trial=None):
	...
	for ThisTrainStim in range(NumAStim): #compare the stimulus to be classified with the As
		...
		if args.forgetting_function:
			...
			SimA += (np.exp(-sens * sum(DistA)) * ((trial - last_seen[stim] + 1) ** (-1 * beta)))
		else:
	...
	for ThisTrainStim in range((NumAStim), NumTrainStim): #compare the stimulus to be classified with the Bs
		...
		if args.forgetting_function:
			assert last_seen[stim] >= 0
			SimB += (np.exp(-sens * sum(DistB)) * ((trial - last_seen[stim] + 1) ** (-1 * beta)))
		else:
...
for response in responses:
	...
	for trial in range(0, NUM_TRIALS):
		...
		if last_seen[stim_list[trial]] == -1:
			last_seen[stim_list[trial]] = trial
		...
		for iteration in range(10):
			...
			if model == 0:
				if args.response_scaling:
					...
					res = minimize(resid_exemp_response_scaling, params, args=data, constraints=cons, bounds=bounds_response_scaling)
				else:
	...
	# make the 7 bins, depending on where the object is along the power law curve
	bins = [[], [], [], [], [], [], [], [], [], [], []]
	...
	for bin in bins:
		...
		for trial in bin:
			...
			if (vals[trial] == 1):
				answer = 1
			if args.forgetting_function:
				res_exemp += resid_calc_exemp(curr_argmin_exemp, stim_list[trial], answer, seen, last_seen_history[trial], trial)
			else:
