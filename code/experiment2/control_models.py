# Usage: python control_models.py <num_exceptions>
#			where num_exceptions is either 0, 2, or 4
# Ex:	 python control_models.py 4
...
if len(sys.argv) < 2:
	raise ValueError("Missing number-of-exceptions command-line argument")

if sys.argv[1] not in ('0', '2', '4'):
	raise ValueError("Number of exceptions for Experiment 2 must either equal 0, 2, or 4")

# identify the indicator to use based on which condition the user selected
num_exceptions = int(sys.argv[1])
indicator = -1
if num_exceptions == 0:
	indicator = 2
elif num_exceptions == 2:
	indicator = 4
elif num_exceptions == 4:
	indicator = 0
...
Stim = np.loadtxt(f"../helper/6dNLSStim_{num_exceptions}.txt",  delimiter=" ")
...
for response in responses:
	csv_num = response[0]
	# (csv_num - 100) % 6 is the indicator for which of the 6 conditions participant belongs to
	if (csv_num - 100) % 6 != indicator:
		continue
	stim_list = []
	...
	count += 1
	if count % 5 == 0:
		print(f"{count} participant models fitted")
